...
Процедура ПриОткрытии(Отказ)
	...
	Если НЕ ОбъектКаталогаEDT.Существует() Тогда
		...
		ПараметрыКонфигурацияВФайлы = СтрШаблон("DESIGNER /DumpConfigToFiles %1"
			,ПутьДляКоманды(КаталогИБ + "\export"));
		...
		КомандаСоздатьРабочийКаталог= СтрШаблон("cmd.exe /c ring edt%1 workspace import --configuration-files %2 --workspace-location %3 --project-name main"
			,СуффиксВерсииEDT()
			,ПутьДляКоманды(КаталогИБ + "\export\DataProcessors")
			,ПутьДляКоманды(КаталогИБ + "\edt"));
...
Функция СоздатьОбработкуСравнения(КаталогИБ, ИмяФайла1, ИмяФайла2, Заголовок, Отказ)
	...
	Если Файл.Существует()
		...
		КопироватьФайл(ИмяФайла1, КаталогИБ + "\edt\main\src\ExternalDataProcessors\СравнениеТабличныхДокументов\Templates\Макет1\Template.mxlx");
	КонецЕсли;
	...
	Если Файл.Существует()
		...
		КопироватьФайл(ИмяФайла2, КаталогИБ + "\edt\main\src\ExternalDataProcessors\СравнениеТабличныхДокументов\Templates\Макет2\Template.mxlx");
	КонецЕсли;
	...
	КомандаСоздатьОбработку = СтрШаблон("DESIGNER /LoadExternalDataProcessorOrReportFromFiles %1 %2"
		,ПутьДляКоманды(КаталогИБ + "\temp\ExternalDataProcessors\СравнениеТабличныхДокументов.xml")
